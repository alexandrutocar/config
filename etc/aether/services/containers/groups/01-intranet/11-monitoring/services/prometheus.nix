...
# ────────────────────────────────────────────────────────────────────────
{
  ...
  inherit (container) intranet-dns intranet-gateway internet-uptime internet-harmonia internet-gateway;
in {
  services.prometheus = {
    enable = true;
    ...
    scrapeConfigs = [
      ...
      }
      {
        ...
        static_configs = [
          {
            targets = ["${internet-uptime.localAddress}:8080"];
          }
        ];
        basic_auth = {
          username = "-";
          password_file = "/etc/prometheus/scrapers/uptime/api.key";
        };
      }
      {
        job_name = "nginx";
      ...
      }
      {
        ...
        static_configs = [
          {
            targets = ["${internet-harmonia.localAddress}:8080"];
          }
        ];
      }
    ];
